...
import { getInitialSettings, saveLocalSettings, sendNotification } from './lib/store';
...
export default function App() {
  ...
  const [googleToken, setGoogleToken] = useState<string | null>(() => localStorage.getItem('cf_google_token'));
  ...
  // --- Google Drive & PWA Effects ---

  useEffect(() => {
    if (googleToken) {
      localStorage.setItem('cf_google_token', googleToken);
    } else {
      localStorage.removeItem('cf_google_token');
    }
  }, [googleToken]);

  const handleGoogleAuth = (callback?: (token: string) => void) => {
    const client = (window as any).google.accounts.oauth2.initTokenClient({
      ...
      callback: (response: any) => {
        if (response.access_token) {
          ...
          // Share token if user is Saad
          if (user?.key === 'saad') {
            updateDoc(doc(db, 'settings', 'global'), {
              sharedGoogleToken: response.access_token
            }).catch(err => console.error('Failed to share token:', err));
          }
      ...
      const setupPushNotifications = async () => {
        try {
          ...
          if (permission === 'granted') {
            ...
            if (messaging && settings.vapidKey) {
  ...
  }, [firebaseUser, user, settings.vapidKey]);
    ...
    const unsubIdeas = onSnapshot(query(collection(db, 'ideas'), orderBy('createdAt', 'desc')), (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added') {
          const idea = change.doc.data() as Idea;
          if (idea.addedBy !== user.key && Date.now() - idea.createdAt < 10000) {
            new Notification(`New Idea: ${idea.title}`, {
              body: `Added by ${USERS[idea.addedBy]?.name || 'someone'}`,
              icon: '/favicon.ico'
            });
          }
        }
    ...
    const unsubPipeline = onSnapshot(query(collection(db, 'pipeline'), orderBy('updatedAt', 'desc')), (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'modified') {
          const item = change.doc.data() as PipelineItem;
          if (item.movedBy !== user.key && Date.now() - item.movedAt < 10000) {
            new Notification(`Pipeline Update: ${item.title}`, {
              body: `Moved to ${item.stage.charAt(0).toUpperCase() + item.stage.slice(1)} by ${USERS[item.movedBy]?.name || 'someone'}`,
              icon: '/favicon.ico'
            });
          }
        }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col md:flex-row">
      ...
      <aside className="hidden md:flex w-[220px] flex-col border-right border-[#2a2a2a] bg-[#0a0a0a] sticky top-0 h-screen p-4 space-y-8">
        <div className="flex items-center gap-3 px-2">
          ...
          <nav className="flex-1 space-y-1">
            ...
            <NavItem active={activeTab === 'library'} onClick={() => setActiveTab('library')} icon={<Folder size={20} />} label="Library" />
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-[#2a2a2a] flex items-center justify-between px-4 py-3 z-50">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <Bell size={18} className={settings.notificationsEnabled ? "text-green-500" : "text-zinc-600"} />
      ...
      <main className="flex-1 p-4 md:p-8 pb-24 md:pb-8 pt-20 md:pt-8 max-w-6xl mx-auto w-full">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'pipeline' && <Pipeline user={user} data={data} setData={setData} settings={settings} addToast={addToast} googleToken={googleToken || settings.sharedGoogleToken} isShared={!googleToken && !!settings.sharedGoogleToken} handleGoogleAuth={handleGoogleAuth} />}
            {activeTab === 'library' && <Library user={user} googleToken={googleToken || settings.sharedGoogleToken} isShared={!googleToken && !!settings.sharedGoogleToken} handleGoogleAuth={handleGoogleAuth} addToast={addToast} />}
            {activeTab === 'settings' && <SettingsPage user={user} data={data} setData={setData} settings={settings} setSettings={setSettings} addToast={addToast} googleToken={googleToken} setGoogleToken={setGoogleToken} handleGoogleAuth={handleGoogleAuth} />}
          </motion.div>
        </AnimatePresence>
      </main>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0a0a0a]/80 backdrop-blur-xl border-t border-[#2a2a2a] flex justify-around p-2 z-50">
        ...
        <TabItem active={activeTab === 'library'} onClick={() => setActiveTab('library')} icon={<Folder size={20} />} />
      ...
      <div className="hidden md:block fixed bottom-8 right-8 z-50">
        <div className={`p-4 bg-[#111] border border-[#2a2a2a] rounded-2xl shadow-2xl flex items-center gap-3 transition-all ${settings.notificationsEnabled ? 'opacity-100' : 'opacity-50'}`}>
          <div className="relative">
            <Bell size={20} className={settings.notificationsEnabled ? "text-green-500" : "text-zinc-600"} />
            {settings.notificationsEnabled && (
              <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-[#111] animate-pulse"></span>
            )}
          </div>
          <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">
            Notifications {settings.notificationsEnabled ? 'Active' : 'Disabled'}
          </div>
        </div>
      </div>
        ...
        <AnimatePresence>
          {toasts.map(t => (
            ...
            >
              ...
              {t.type === 'info' && <Bell className="text-blue-500 shrink-0" />}
              <div>
...
function Ideation({ user, data, setData, settings, addToast }: any) {
  ...
  const handleAddIdea = async () => {
    ...
    try {
      ...
      if (settings.notificationsEnabled && settings.ntfyTopic) {
        sendNotification(settings.ntfyTopic, 'New Idea Added', `${user.name} added: ${idea.title}`);
      }
    } catch (err) {
  ...
  const handleApprove = async (idea: Idea) => {
    try {
      ...
      if (settings.notificationsEnabled && settings.ntfyTopic) {
        sendNotification(settings.ntfyTopic, 'Idea Approved', `${user.name} approved: ${idea.title}`);
      }
    } catch (err) {
...
function Pipeline({ user, data, setData, settings, addToast, googleToken, isShared, handleGoogleAuth }: any) {
  ...
  const handleMove = (item: PipelineItem, direction: 'forward' | 'backward') => {
    ...
    if (direction === 'forward') {
      ...
      // File upload requirement for specific stages
      if ((item.stage === 'scripting' || item.stage === 'editing')) {
        if (!googleToken) {
          handleGoogleAuth(() => handleMove(item, 'forward'));
          return;
        }
        ...
        input.onchange = async (e: any) => {
          ...
          try {
            const metadata = {
              name: `${item.channel}_${item.title}_${item.stage === 'scripting' ? 'Script' : 'Video'}`,
              mimeType: file.type,
              appProperties: {
                ideaId: item.ideaId,
                stage: item.stage,
            ...
            await updateStage(item.id, nextStage, `Uploaded file: ${file.name}`, result.id);
          } catch (err) {
            addToast('Upload Failed', 'Could not upload to Google Drive.', 'error');
          } finally {
  ...
  const updateStage = async (itemId: string, nextStage: any, message?: string, driveFileId?: string) => {
    try {
      ...
      // (Implementation of FCM notification logic would go here)
...
function Library({ user, googleToken, isShared, handleGoogleAuth, addToast }: any) {
  ...
  const fetchFiles = useCallback(async (token: string) => {
    setIsLoading(true);
    try {
      const response = await fetch('https://www.googleapis.com/drive/v3/files?q=appProperties has { key="ideaId" }&fields=files(id, name, mimeType, size, createdTime, appProperties, webViewLink)', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
      setFiles(data.files || []);
    } catch (err) {
      addToast('Error', 'Failed to fetch files from Drive.', 'error');
    } finally {
      setIsLoading(false);
    }
  }, [addToast]);
  ...
  useEffect(() => {
    if (googleToken) fetchFiles(googleToken);
  }, [googleToken, fetchFiles]);
  ...
  const handleDelete = async (fileId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addToast('Deleted', 'File removed from Drive.', 'success');
      }
    } catch (err) {
  ...
  if (!googleToken) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center space-y-6 text-center">
        <div className="w-20 h-20 bg-[#111] border border-[#2a2a2a] rounded-3xl flex items-center justify-center text-zinc-500">
          <Cloud size={40} />
        </div>
        <div className="space-y-2">
          <h2 className="text-2xl font-bold">Connect Google Drive</h2>
          <p className="text-sm text-zinc-500 max-w-xs">Access and manage your scripts and videos directly from Drive.</p>
        </div>
        <button 
  ...
  const filteredFiles = files.filter(f => filter === 'all' || f.appProperties?.channel === filter);
    ...
    <div className="space-y-8">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-sm text-zinc-500">Manage all uploaded scripts and videos.</p>
          </div>
          {isShared && (
            <div className="bg-blue-500/10 border border-blue-500/20 px-3 py-1 rounded-full flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></div>
        ...
        <div className="flex gap-2 bg-[#111] p-1 rounded-lg border border-[#2a2a2a]">
          {['all', ...DEFAULT_CHANNELS.map(c => c.id)].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-40 bg-[#111] border border-[#2a2a2a] rounded-xl animate-pulse" />
          ))}
        </div>
      ) : filteredFiles.length === 0 ? (
        <div className="h-40 flex items-center justify-center border border-dashed border-[#2a2a2a] rounded-xl text-zinc-500 text-sm">
          No files found for this filter.
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredFiles.map(file => (
            <div key={file.id} className="bg-[#111] border border-[#2a2a2a] rounded-xl p-5 space-y-4 group relative overflow-hidden">
              <div className="flex items-start justify-between">
                <div className="p-3 bg-[#0a0a0a] border border-[#2a2a2a] rounded-xl text-white">
                  {file.mimeType.includes('video') ? <Video size={20} /> : <FileText size={20} />}
                </div>
                <div className="flex gap-1">
                  <span className="text-[8px] font-bold px-2 py-1 rounded bg-zinc-800 text-zinc-400 uppercase tracking-widest">
                    {file.appProperties?.stage}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-zinc-500">
                  {(parseInt(file.size) / (1024 * 1024)).toFixed(2)} MB • {new Date(file.createdTime).toLocaleDateString()}
                </p>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <a 
                  ...
                  className="flex-1 bg-[#1a1a1a] text-white text-[10px] font-bold py-2 rounded-lg text-center hover:bg-zinc-800 transition-colors flex items-center justify-center gap-2"
                >
...
function SettingsPage({ user, data, setData, settings, setSettings, addToast, googleToken, setGoogleToken, handleGoogleAuth }: any) {
    ...
    try {
      ...
      if (newValue) {
        if ('Notification' in window) {
          const permission = await Notification.requestPermission();
          if (permission !== 'granted') {
            addToast('Permission Denied', 'Please enable notifications in your browser settings.', 'error');
          }
        }
      }
  ...
  const updateNtfyTopic = async (topic: string) => {
    try {
      await updateDoc(doc(db, 'settings', 'global'), {
        ntfyTopic: topic
      });
  ...
  const updateVapidKey = async (key: string) => {
    try {
      await updateDoc(doc(db, 'settings', 'global'), {
        vapidKey: key
      });
        ...
        <div className="bg-[#111] border border-[#2a2a2a] rounded-xl p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xs text-zinc-500">Receive real-time alerts for new ideas and pipeline moves.</p>
            </div>
            <button 
              onClick={toggleNotification}
              className={`w-12 h-6 rounded-full transition-colors relative ${settings.notificationsEnabled ? 'bg-green-500' : 'bg-zinc-800'}`}
            >
              <motion.div 
        ...
        <div className="bg-[#111] border border-[#2a2a2a] rounded-xl p-6 space-y-6">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-zinc-500">
                {googleToken 
                  ...
                  : settings.sharedGoogleToken 
                    ? 'Using shared connection from Saad.' 
                    : 'Not connected to Google Drive.'}
              </p>
            </div>
            <div className="flex items-center gap-3">
              {(googleToken || settings.sharedGoogleToken) ? (
                <div className="flex items-center gap-2 text-green-500 text-xs font-bold bg-green-500/10 px-3 py-1 rounded-full border border-green-500/20">
                  <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
                  Connected
                </div>
              ) : (
                <div className="flex items-center gap-2 text-zinc-500 text-xs font-bold bg-zinc-500/10 px-3 py-1 rounded-full border border-zinc-500/20">
                  Disconnected
                </div>
              )}
            </div>
          ...
          <div className="flex gap-4">
            {!googleToken && (
              <button 
                onClick={() => handleGoogleAuth()}
                className="flex-1 bg-white text-black font-bold py-3 rounded-lg hover:bg-zinc-200 transition-colors flex items-center justify-center gap-2"
              >
                <Upload size={18} />
                {settings.sharedGoogleToken ? 'Connect Own Drive' : 'Connect Google Drive'}
              </button>
            )}
            {googleToken && (
              <button 
                onClick={() => {
                  ...
                  if (user.key === 'saad') {
                    ...
                    }).catch(err => console.error('Failed to clear shared token:', err));
                  }
                  addToast('Disconnected', 'Google Drive connection removed.', 'info');
                }}
                className="flex-1 bg-[#1a1a1a] text-white font-bold py-3 rounded-lg hover:bg-zinc-800 transition-colors"
              >
                Disconnect
